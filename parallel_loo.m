...
for i = 1:N
    ...
    s1 = ['f{i} = ftrain.t',num2str(i),';'];
    s2 = ['an{i} = atrain.t',num2str(i),';'];
    s3 = ['t{i} = ftest.t',num2str(i),';'];
    eval(s1);
    eval(s2);
    eval(s3);
%     index1 = rand(1,l(i,1));
...
%             eval(s1);
            svmModel = svmtrain(f{i}((a-1)*TO+1:(a*TO),:),an{i}((a-1)*TO+1:(a*TO)),'kernel_function','rbf','options',options);
        end


%         s2 = ['svm_pre.p',num2str(i),' = svmclassify(svmModel',num2str(i),',ftest.t',num2str(i),');'];
%         eval(s2);
        p{i} = svmclassify(svmModel,t{i});

end


for i = 1:N
    s101 = ['svm_pre.p',num2str(i),' = p{i};'];
