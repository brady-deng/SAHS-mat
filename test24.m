...
%% 数据分析
num2.data = zeros(N+1,n_sta);
num2.header{1,1} = 'totalnumber_healthy';
num2.header{1,2} = 'totalnumber_ill';
num2.header{1,3} = 'trainnumber_healthy';
num2.header{1,4} = 'trainnumber_ill';
num2.header{1,5} = 'testnumber_healthy';
num2.header{1,6} = 'testnumber_ill';
num2.header{1,7} = 'truenegative';
num2.header{1,8} = 'truepositive';
num2.header{1,9} = 'true-predict';
num2.header{1,10} = 'false-predict';
num2.header{1,11} = 'true-porpotion';
num2.header{1,12} = 'true-positive';
num2.header{1,13} = 'true-negative';

for i = 1:N
    for a = 1:data_size(i,2)
        s1 = ['if output.ou',num2str(i),'(a)>0 num2.data(i,2) = num2.data(i,2)+1;else num2.data(i,1) = num2.data(i,1)+1; end'];

        eval(s1);

    end
    for a = 1:data_size(i,4)
        s2 = ['if output_train.ou',num2str(i),'(1,a)>0 num2.data(i,4) = num2.data(i,4)+1;else num2.data(i,3) = num2.data(i,3)+1; end'];
        eval(s2);
    end
    for a = 1:data_size(i,6)
        s3 = ['if output_test.ou',num2str(i),'(1,a)>0 num2.data(i,6) = num2.data(i,6)+1; else num2.data(i,5) = num2.data(i,5)+1; end'];
        eval(s3);
    end
end
% n_health = 0;
...
for i = 1:N
    s1 = ['net',num2str(i),'=newff(minmax(inputn.n',num2str(i),'),[5,2]);'];
...
for i = 1:N
    for a = 1: data_size(i,6)
        s1 = ['if Pout.n',num2str(i),'(1,',num2str(a),')==output_test.ou',num2str(i),'(1,',num2str(a),')&&Pout.n',num2str(i),'(1,',num2str(a),')==1 num2.data(',num2str(i),',8) = num2.data(',num2str(i),',8)+1; end' ];
        s2 = ['if Pout.n',num2str(i),'(1,',num2str(a),')==output_test.ou',num2str(i),'(1,',num2str(a),')&&Pout.n',num2str(i),'(1,',num2str(a),')==0 num2.data(',num2str(i),',7) = num2.data(',num2str(i),',7)+1; end' ];
        s3 = ['if Pout.n',num2str(i),'(1,',num2str(a),')==output_test.ou',num2str(i),'(1,',num2str(a),') num2.data(',num2str(i),',9) = num2.data(',num2str(i),',9)+1; end' ];
        s4 = ['if Pout.n',num2str(i),'(1,',num2str(a),')~=output_test.ou',num2str(i),'(1,',num2str(a),') num2.data(',num2str(i),',10) = num2.data(',num2str(i),',10)+1; end' ];
    ...
    num2.data(i,11) = 100*num2.data(i,9)/(num2.data(i,5)+num2.data(i,6));
    num2.data(i,12) = 100*num2.data(i,8)/num2.data(i,6);
    num2.data(i,13) = 100*num2.data(i,7)/num2.data(i,5);
end


if N == 1
    num2.data(N+1,:) = num2.data(1:N,:);
else
    num2.data(N+1,:) = sum(num2.data(1:N,:));
    num2.data(N+1,11) = 100*num2.data(N+1,9)/(num2.data(N+1,5)+num2.data(N+1,6));
    num2.data(N+1,12) = 100*num2.data(N+1,8)/num2.data(N+1,6);
    num2.data(N+1,13) = 100*num2.data(N+1,7)/num2.data(N+1,5);
end


disp('正确率：');
disp(num2.data(N+1,11));
disp('正阳率：');
disp(num2.data(N+1,12));
disp('正阴率：');
disp(num2.data(N+1,13));
